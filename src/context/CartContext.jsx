...
export function CartProvider({ children }) {
  ...
  // ✅ تعديل addToCart لاستقبال selectedSize اختياري
  const addToCart = (item, selectedSize = null) => {
    setCartItems((prev) => {
      const cartItem = {
        ...
        cartId: Date.now() + Math.random(),
        quantity: 1,
        selectedSize: selectedSize || null, // ✅ تخزين الحجم المختار (إن وجد)
        originalDescription: item.description ? [...item.description] : [],
        customizations: {
          removedDescs: []
        }
  ...
  // ✅ تعديل حساب السعر لاستخدام سعر الحجم إذا وجد
  const totalPrice = cartItems.reduce(
    (sum, item) =>
      sum + (item.selectedSize?.price || item.price || 0) * item.quantity,
    0
  );
